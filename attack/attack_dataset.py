...
from mf_attack.guass.guass import get_gaussian_kernel
...
class AttackDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # torch.clamp(input, min, max, outr=None):限制范围界限
        attack_mask = torch.clamp(self.attack_masks[index], -25.5, 25.5)
        attack_mask_unsqueeze = torch.unsqueeze(attack_mask, dim=0)
        blur_layer = get_gaussian_kernel().cuda()
        blured_mask = blur_layer(attack_mask_unsqueeze)
        blured_mask = torch.squeeze(blured_mask)
        img_after_attack = img.clone()
        img_after_attack[:, self.y:self.y + self.h, self.x:self.x + self.w] = img_after_attack[:, self.y:self.y + self.h, self.x:self.x + self.w] + blured_mask
